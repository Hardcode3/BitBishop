...
TEST(FlipIndex, IsInvolution) {
  for (std::size_t i = 0; i < Const::BOARD_SIZE; ++i) {
    const std::size_t flipped_index = flip_index_horizontally(i);
    EXPECT_EQ(i, flip_index_horizontally(flipped_index)) << "at index " << i;
  }
}
...
TEST(FlipIndex, PreservesFileAndFlipsRank) {
  ...
  for (std::size_t rank = 0; rank < BOARD_WIDTH; ++rank) {
    for (std::size_t file = 0; file < BOARD_WIDTH; ++file) {
      ...
      std::size_t j = flip_index_horizontally(i);
...
TEST(FlipIndex, FlipsCornersCorrectly) {
  EXPECT_EQ(flip_index_horizontally(0), 56);  // A8 -> A1
  EXPECT_EQ(flip_index_horizontally(7), 63);  // H8 -> H1
  EXPECT_EQ(flip_index_horizontally(56), 0);  // A1 -> A8
  EXPECT_EQ(flip_index_horizontally(63), 7);  // H1 -> H8
}